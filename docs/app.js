...
async function resetDispositivo(){
  ...
  // borra caches de la PWA (si existen)
  if (window.caches) {
    const keys = await caches.keys();
    await Promise.all(keys.map(k => caches.delete(k)));
  }
  // (opcional) indexedDB (si usaste bases propias de la app)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const $vMsg   = $('#verif-msg');       // estado VALIDACIÓN (pestaña FICHAJE)
  const $fMsg   = $('#fichaje-msg');     // estado envío FICHAJE
  const $ult    = $('#ultimo-fichaje');  // resumen último fichaje
  const $datosMsg = $('#datos-valid-msg'); // estado VALIDACIÓN (pestaña DATOS)
  const $btnReset = $('#btn-reset');     // borrar datos
    ...
    valTimer = setTimeout(async ()=>{
      ...
      try{
        ...
        if(ok){
          ...
          // Sugerir tipo según último del mismo trabajador
